...
public class AletheiaCommonEventHandler {
    ...
    public void onItemCrafted(PlayerEvent.ItemCraftedEvent event) {
        ...
        if (crafted.isItemEqual(new ItemStack(AletheiaBlocks.ams_base))) {
            ...
            if (!alreadyUnlocked) {
                ...
                player.worldObj.spawnEntityInWorld(
                    new EntityLightningBolt(player.worldObj, player.posX, player.posY, player.posZ));
                player.worldObj.spawnEntityInWorld(
        ...
        } else if (crafted.getItem() == AletheiaItems.gun_pppop) {
            Achievement achievement = AletheiaAchievements.achievementPPPOP;
            boolean unlockedNow = grantAchievement(player, achievement);
            if (!unlockedNow) {
                ExplosionNukeSmall
                    .explode(player.worldObj, player.posX, player.posY, player.posZ, ExplosionNukeSmall.PARAMS_MEDIUM);
            }
        }
    }
    ...
    public static boolean grantAchievement(EntityPlayer player, Achievement achievement) {
        ...
        if (player instanceof EntityPlayerMP) {
            ...
            if (playerMP.func_147099_x() != null && !playerMP.func_147099_x()
                .hasAchievementUnlocked(achievement)) {
                unlockedNow = true;
            }
        }
        player.triggerAchievement(achievement);
        if (unlockedNow && achievement == AletheiaAchievements.achievementPPPOP) {
            ExplosionNukeSmall
                .explode(player.worldObj, player.posX, player.posY, player.posZ, ExplosionNukeSmall.PARAMS_MEDIUM);
        }
        return unlockedNow;
