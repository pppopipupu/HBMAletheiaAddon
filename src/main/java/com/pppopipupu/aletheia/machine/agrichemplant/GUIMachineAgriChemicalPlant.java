...
public class GUIMachineAgriChemicalPlant extends GuiInfoContainer {

    private static ResourceLocation texture = new ResourceLocation(
        Aletheia.MODID + ":textures/gui/processing/gui_agri_chemplant.png");
    private TileEntityMachineAgriChemicalPlant chemplant;

    public GUIMachineAgriChemicalPlant(InventoryPlayer invPlayer, TileEntityMachineAgriChemicalPlant tedf) {
        super(new ContainerMachineAgriChemicalPlant(invPlayer, tedf));
        chemplant = tedf;
    ...
    public void drawScreen(int mouseX, int mouseY, float f) {
        ...
        if (guiLeft + 7 <= mouseX && guiLeft + 7 + 18 > mouseX
            && guiTop + 125 < mouseY
            && guiTop + 125 + 18 >= mouseY) {
            if (this.chemplant.chemplantModule.getRecipe() != null && AgriChemicalPlantRecipes.INSTANCE.recipeNameMap
                .containsKey(this.chemplant.chemplantModule.getRecipeName())) {
                ...
                this.drawCreativeTabHoveringText(
                    EnumChatFormatting.YELLOW + I18nUtil.resolveKey("gui.recipe.setRecipe"),
                    mouseX,
                    mouseY);
            }
        }
    ...
    protected void mouseClicked(int x, int y, int button) {
        ...
        if (this.checkClick(x, y, 7, 125, 18, 18)) GUIScreenRecipeSelector.openSelector(
            AgriChemicalPlantRecipes.INSTANCE,
            chemplant,
            chemplant.chemplantModule.getRecipeName(),
            0,
            ItemBlueprints.grabPool(chemplant.slots[1]),
            this);
    }

    @Override
    protected void drawGuiContainerForegroundLayer(int i, int j) {
        String name = this.chemplant.hasCustomInventoryName() ? this.chemplant.getInventoryName()
            : I18nUtil.resolveKey(this.chemplant.getInventoryName());
    ...
    protected void drawGuiContainerBackgroundLayer(float p_146976_1_, int p_146976_2_, int p_146976_3_) {
        ...
        if (chemplant.chemplantModule.progress > 0) {
            ...
            drawTexturedModalRect(
                ...
                guiTop + 126,
                176,
                61 + (chemplant.chemplantModule.restrictedMode ? 16 : 0),
                j,
                16);
        }
        ...
        if (recipe != null && recipe.inputItem != null) {
            for (int i = 0; i < recipe.inputItem.length; i++) {
                ...
                if (!slot.getHasStack()) this.renderItem(
            ...
            for (int i = 0; i < recipe.inputItem.length; i++) {
                Slot slot = (Slot) this.inventorySlots.inventorySlots.get(chemplant.chemplantModule.inputSlots[i]);
                if (!slot.getHasStack()) drawTexturedModalRect(
