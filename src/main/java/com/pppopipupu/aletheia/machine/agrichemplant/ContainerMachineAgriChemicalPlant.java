...
public class ContainerMachineAgriChemicalPlant extends ContainerBase {

    public ContainerMachineAgriChemicalPlant(InventoryPlayer invPlayer, IInventory chemicalPlant) {
        super(invPlayer, chemicalPlant);

        // Battery
        this.addSlotToContainer(new SlotNonRetarded(chemicalPlant, 0, 152, 81));
        // Blueprint / template (unused, auto-recipe-select) + upgrades. These MUST be added in
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (index <= tile.getSizeInventory() - 1) {
                ...
                if (slotOriginal.getItem() instanceof IBatteryItem
                    || slotOriginal.getItem() == ModItems.battery_creative) {
