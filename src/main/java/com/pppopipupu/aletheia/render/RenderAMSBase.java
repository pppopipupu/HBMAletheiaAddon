...
public class RenderAMSBase extends TileEntitySpecialRenderer implements IItemRendererProvider {
    ...
    public static final ResourceLocation ams_base_tex = new ResourceLocation(
        Aletheia.MODID,
        "textures/models/ams_base.png");
    public static final IModelCustom ams_base = new HFRWavefrontObject(
        new ResourceLocation(Aletheia.MODID, "models/ams_base.obj")).asVBO();
    ...
    public void renderTileEntityAt(TileEntity tile, double x, double y, double z, float interp) {
        ...
        if (isPPPOP) {
            ...
            boolean hasCatalysts = ams.slots[8] != null && ams.slots[8].getItem() instanceof ItemCatalyst
                && ams.slots[9] != null
                && ams.slots[9].getItem() instanceof ItemCatalyst
                && ams.slots[10] != null
                && ams.slots[10].getItem() instanceof ItemCatalyst
                && ams.slots[11] != null
                && ams.slots[11].getItem() instanceof ItemCatalyst;

            boolean hasCore = ams.slots[12] != null && ams.slots[12].getItem() instanceof ItemAMSCore;

            boolean isRunning = ams.field > 0 && ams.efficiency > 0
                && hasCatalysts
                && hasCore
                && ams.deuterium.getFill() > 0
                && ams.tritium.getFill() > 0;
    ...
    private void renderGreenOrb(TileEntityAMSBase tile, double x, double y, double z) {
        ...
        double ix = (tile.getWorldObj()
            .getTotalWorldTime() * 0.15D) % (Math.PI * 2D);
        double t = 0.8F;
    ...
    private void renderOrb(TileEntityAMSBase tile, double x, double y, double z) {
        ...
        bindTexture(ResourceManager.fusion_plasma_sparkle_tex);
        double ix = (tile.getWorldObj()
            .getTotalWorldTime() * 0.1D) % (Math.PI * 2D);
        double t = 0.8F;
        float pulse = (float) ((1D / t) * Math.atan((t * Math.sin(ix)) / (1 - t * Math.cos(ix))));
        pulse = (pulse + 1F) / 2F;
