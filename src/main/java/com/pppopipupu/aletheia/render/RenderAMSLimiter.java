...
public class RenderAMSLimiter extends TileEntitySpecialRenderer implements IItemRendererProvider {
    ...
    public static final ResourceLocation ams_limiter_tex = new ResourceLocation(
        Aletheia.MODID,
        "textures/models/ams_limiter.png");
    public static final IModelCustom ams_limiter = new HFRWavefrontObject(
        new ResourceLocation(Aletheia.MODID, "models/ams_limiter.obj")).asVBO();
    ...
    private void renderMagneticBeam(TileEntityAMSLimiter limiter, double x, double y, double z) {
        ...
        for (ForgeDirection d : axes) {
            for (int r = 1; r <= 8; r++) {
                TileEntity te = limiter.getWorldObj()
                    .getTileEntity(limiter.xCoord + d.offsetX * r, limiter.yCoord, limiter.zCoord + d.offsetZ * r);
